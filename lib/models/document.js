...
class Document {
    ...
    static async fromCSV(headers, data, opts) {
        ...
        for (const doc of data) {
            ...
            for (let index = 0; index < headers.length; index++) {
                ...
                switch (header) {
                    case 'allowAnonymousAccess':
                        ...
                        break;
                    case 'title':
                        document.setProperty('title', value)
                        break;
                    case 'summary':
                        document.setProperty('summary', { mimeType: MIME_TYPES.TEXT, textContent: value })
                        break;
                    case 'viewURL':
                        if (fetchViewUrl) {
                        ...
                        document.setViewURL(value)
                        break;
                    case 'tags':
                        document.setProperty('tags', value.split(','))
                        break;
                    case object_type:
                        if(OBJECT_TYPES.isValueValid(value))
                        document.objectType = value
                        break;
                    default:
            ...
            document.objectType = OBJECT_TYPES.ACRONYMS
